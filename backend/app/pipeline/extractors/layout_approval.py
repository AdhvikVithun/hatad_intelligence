"""Layout Approval extractor — text-based."""
...
from app.pipeline.schemas import EXTRACT_LAYOUT_SCHEMA

logger = logging.getLogger(__name__)


class LayoutApprovalExtractor(BaseExtractor):
    """Extract approval details and plot schedule from Layout Approval documents."""

    def __init__(self):
        self.system_prompt = (PROMPTS_DIR / "extract_layout_approval.txt").read_text(encoding="utf-8")
    ...
    async def extract(
        ...
        **kwargs,
        ...
        name = filename or "Layout Approval"
    ...
    async def _extract_single(self, text: str, label: str, on_progress) -> dict:
        prompt = f"Extract all details from this Layout Approval document:\n\n{text}"
        return await call_llm(
            prompt=prompt,
            system_prompt=self.system_prompt,
            expect_json=EXTRACT_LAYOUT_SCHEMA,
    ...
    def _merge(results: list) -> dict:
        ...
        for r in results:
            if isinstance(r, Exception) or not isinstance(r, dict):
                continue
            all_plots.extend(r.pop("plot_schedule", []))
            all_conditions.extend(r.pop("conditions", []))
            for k, v in r.items():
                ...
                if existing is None or existing == "" or existing == [] or existing == {}:
                    merged[k] = v
        if all_plots:
            merged["plot_schedule"] = all_plots
        if all_conditions:
            # Deduplicate conditions
            merged["conditions"] = list(dict.fromkeys(all_conditions))
        return merged
