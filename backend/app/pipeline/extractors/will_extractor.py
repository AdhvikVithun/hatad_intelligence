"""Will (உயில்) extractor — text-based."""
...
from app.pipeline.schemas import EXTRACT_WILL_SCHEMA

logger = logging.getLogger(__name__)


class WillExtractor(BaseExtractor):
    """Extract testator, beneficiaries, executor, and probate status from Wills."""

    def __init__(self):
        self.system_prompt = (PROMPTS_DIR / "extract_will.txt").read_text(encoding="utf-8")
    ...
    async def _extract_single(self, text: str, label: str, on_progress) -> dict:
        prompt = f"Extract all details from this Will document:\n\n{text}"
        return await call_llm(
            prompt=prompt,
            system_prompt=self.system_prompt,
            expect_json=EXTRACT_WILL_SCHEMA,
    ...
    def _merge(results: list) -> dict:
        ...
        for r in results:
            if isinstance(r, Exception) or not isinstance(r, dict):
                continue
            all_beneficiaries.extend(r.pop("beneficiaries", []))
            all_codicils.extend(r.pop("codicils", []))
            for k, v in r.items():
        ...
        if all_beneficiaries:
            merged["beneficiaries"] = all_beneficiaries
        if all_codicils:
            merged["codicils"] = all_codicils
        return merged
