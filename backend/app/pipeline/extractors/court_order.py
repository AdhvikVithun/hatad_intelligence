"""Court Order extractor — text-based."""
...
from app.pipeline.schemas import EXTRACT_COURT_ORDER_SCHEMA

logger = logging.getLogger(__name__)


class CourtOrderExtractor(BaseExtractor):
    """Extract case details, order type, and property restrictions from Court Orders."""

    def __init__(self):
        self.system_prompt = (PROMPTS_DIR / "extract_court_order.txt").read_text(encoding="utf-8")
    ...
    async def _extract_single(self, text: str, label: str, on_progress) -> dict:
        prompt = f"Extract all details from this Court Order:\n\n{text}"
        return await call_llm(
            prompt=prompt,
            system_prompt=self.system_prompt,
            expect_json=EXTRACT_COURT_ORDER_SCHEMA,
