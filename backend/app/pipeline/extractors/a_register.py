"""A-Register (அ-பதிவேடு) extractor — text-based."""
...
from app.pipeline.schemas import EXTRACT_A_REGISTER_SCHEMA

logger = logging.getLogger(__name__)


class ARegisterExtractor(BaseExtractor):
    """Extract mutation history and ownership chain from A-Register records."""

    def __init__(self):
        self.system_prompt = (PROMPTS_DIR / "extract_a_register.txt").read_text(encoding="utf-8")
    ...
    async def _extract_single(self, text: str, label: str, on_progress) -> dict:
        prompt = f"Extract all details from this A-Register document:\n\n{text}"
    ...
    def _merge(results: list) -> dict:
        ...
        for r in results:
            if isinstance(r, Exception) or not isinstance(r, dict):
                continue
            all_mutations.extend(r.pop("mutation_entries", []))
            all_surveys.extend(r.pop("survey_numbers", []))
            for k, v in r.items():
                ...
                if existing is None or existing == "" or existing == [] or existing == {}:
                    merged[k] = v
        if all_mutations:
            merged["mutation_entries"] = all_mutations
        if all_surveys:
            # Deduplicate survey numbers
            seen = set()
            deduped = []
            for s in all_surveys:
                key = str(s) if isinstance(s, str) else str(s.get("survey_no", s))
                if key not in seen:
                    seen.add(key)
                    deduped.append(s)
            merged["survey_numbers"] = deduped
        return merged
